...
class RepositoryViewSet(viewsets.ModelViewSet):  # pylint: disable=too-many-ancestors
    ...
    def create(self, request, *args, **kwargs):
        ...
        repository = GitService().get_repository(
            repository_name, repository_owner, self.request.user
        )
        if repository:
            ...
            GitService().set_repository_webhook(
                new_repository.name,
                new_repository.owner,
                self.request.user,
                webhook_url
            )
            serialized_repository = RepositorySerializer(
                new_repository,
    ...
    def save_commits(self, repository):
        repository_commits = GitService().get_repository_commits(
            repository.name,
            repository.owner,
            self.request.user
        )
...
class CommitViewSet(viewsets.ModelViewSet):  # pylint: disable=too-many-ancestors
    """
...
class UsersRepositoryList(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        saved_repositories = Repository.objects.filter(
            user=self.request.user
        ...
        if response:
            response = [
                ...
                in response if repository['name']
                not in saved_repositories
            ]
